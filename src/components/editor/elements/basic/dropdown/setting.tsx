...
import { StyleSettings } from "../../typography/h-one/settings";
...
interface Props {
  nodeProps: NodeProps;
  setProp: (callback: (prop: NodeProps) => void, debounce?: number) => void;
}
...
const DropdownSettings = () => {
  ...
  // Use local state to prevent frequent updates
 
  const [title, setTitle] = useState(nodeProps.title || "");
  const [link, setLink] = useState(nodeProps.link || "");
  const [submenus, setSubmenus] = useState<Submenu[]>(nodeProps.submenus || []);
  ...
  // Update a submenu
  const updateSubmenu = (index: number, key: keyof Submenu, value: string) => {
    setSubmenus((prev) =>
      prev.map((submenu, i) => (i === index ? { ...submenu, [key]: value } : submenu))
    );
  };
  ...
  // Save changes & sync to setProp
  const saveChanges = () => {
    setProp((prop) => {
      prop.title = title;
      prop.link = link;
      prop.submenus = submenus;
    }, 500);
  };
  ...
  return (
    <>
          ...
          <>
            {/* <div className="mb-2">
              ...
              <div>
                                <div className="flex items-start p-2">  
                                <div className="text-xs">
                                  <Label className="text-sm">Submenu Name</Label>
                                  <Input
                                    className="h-8 bg-white"
                                    value={nodeProps.submenuTitle}
                                    onChange={(e) => {
                                      setProp((prop) => (prop.title = e.target.value), 500);
                                    }}
                                  ></Input>
      ...
      {/* Add Submenu Button */}
      <Button onClick={addSubmenu} className="mt-2">
        Add Submenu
      </Button>
      ...
      <div>
        {submenus.map((submenu, index) => (
          <div key={index} className="flex relative items-start p-2">
            <div className="text-xs w-full">
              <Label className="text-sm">Submenu Name {index + 1}</Label>
              <Input
                className="h-8 bg-white"
                value={submenu.name || ""}
              ...
              <Label className="text-sm">Submenu Link {index + 1}</Label>
              <Input
                className="h-8 bg-white"
                value={submenu.link || "" }
